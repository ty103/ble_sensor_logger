...
- `/api/capability` は接続中のみCapabilityをReadし、stream metadataをJSONへ整形して返す。現行では `DUMMY_ACCEL3`、`IMU6`、`TEMP_HUMIDITY`、`PRESSURE`、`MAG3` を扱える。
...
| Test | 確認内容 |
| --- | --- |
| `test_protocol.py` | payload size、pack/unpack、validation、v3 Sensor Data frame parse、IMU6/HTS221/LPS22HB/MAG3 payload parse |
| `test_app_core.py` | sample handler、stream単位のsequence欠落検出 |
| `test_web_api.py` | 期待route、Capability metadata JSON、payload外fieldをWeb sample JSONで `null` にすること |
