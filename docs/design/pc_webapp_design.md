...
- `/api/capability` は接続中のみCapabilityをReadし、stream metadataをJSONへ整形して返す。現行では `DUMMY_ACCEL3`、`IMU6`、`TEMP_HUMIDITY`、`PRESSURE`、`MAG3` を扱える。PC backendは次段のLSM6DSL派生姿勢stream `ORIENTATION_MOTION` / `ORIENTATION_MOTION_INT16_V1` もparseとmetadata補完に対応済みで、Firmware/WebGUI統合前のsynthetic Capabilityで検証する。
